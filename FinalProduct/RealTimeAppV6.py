...
class EmotionDetectionModel1(nn.Module):
    
    def __init__(self, img_size, num_classes = 7):
        super(EmotionDetectionModel1, self).__init__()
...
emotion_model = EmotionDetectionModel1(img_size, 8)
checkpoint_path = 'Emotion classification/CKP_model_1_t2.ckpt'  # Path to the trained emotion detection model checkpoint
    ...
    while True:
        ...
        for result in results:
            for box in result.boxes:
                ...
                train_mean = torch.tensor([0.4989])
                train_std = torch.tensor([0.2907])
