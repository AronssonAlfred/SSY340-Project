...
def main():
    ...
    while True:
        ...
        # List to store cropped faces for display
        cropped_faces = []
        ...
        for result in results:
            for box in result.boxes:
                ...
                # Store cropped face for display
                cropped_faces.append(cropped_face_resized)

        # Display the cropped faces in the top-right corner
        for idx, cropped_face in enumerate(cropped_faces):
            # Convert each cropped face to grayscale
            cropped_face_gray = cv2.cvtColor(cropped_face, cv2.COLOR_BGR2GRAY)

            # Convert grayscale image back to BGR so it can be displayed correctly in the frame
            cropped_face_gray_bgr = cv2.cvtColor(cropped_face_gray, cv2.COLOR_GRAY2BGR)

            # Adjust spacing between images and add grayscale cropped face to the frame
            y_offset = idx * (img_size + 10)
            frame[10 + y_offset:10 + y_offset + img_size, -img_size - 10:-10] = cropped_face_gray_bgr
